...
proc execute*(args: seq[string], procobj: ProcApi.ProcTypes.ProcessObject): int =
    ...
    for i in 0..<count:
        ...
        if val < LIMITS.LIM_MINIMUM or val > LIMITS.LIM_MAXIMUM:
            ErrorApi.ThrowError(ErrorApi.newerr("data value exceeds allowed limits", ErrorApi.SysErr.ErrorSeverity.Fatal, ErrorApi.ErrTypes.CATEGORY_CPU, ErrorApi.ErrTypes.CPU_LIMEXC))
